...
const QUESTIONS: Question[] = [
  { broken: '=___(A1:A10)', full: '=SOMME(A1:A10)', hint: 'باغي تجمع نقط يوسف من A1 حتى A10.', options: ['SOMME', 'MOYENNE', 'MAX', 'NB'], correctIndex: 0, explanation: 'SOMME كتجمع كل الأرقام من A1 حتى A10.' },
  { broken: '=SOMME(___:B5)', full: '=SOMME(B1:B5)', hint: 'المجموعة كتبدا من أول خلية فعمود B.', options: ['B1', 'A1', 'B0', 'C1'], correctIndex: 0, explanation: 'باش تجمع B1 حتى B5 خاصك تكتب B1:B5.' },
  { broken: '=___(A1:A10)', full: '=MOYENNE(A1:A10)', hint: 'باغي تحسب المعدل ديال النقط.', options: ['MOYENNE', 'SOMME', 'MIN', 'MAX'], correctIndex: 0, explanation: 'MOYENNE كتجمع النقط وتقسمهم على العدد ديالهم.' },
  { broken: '=SI(A1>10,___, "Non")', full: '=SI(A1>10,"Oui","Non")', hint: 'إيلا الشرط صحيح، Excel كيعطي الاختيار الأول.', options: ['"Oui"', '"Non"', '10', 'A1'], correctIndex: 0, explanation: 'فـ SI كنكتبو: الشرط، جواب إلا كان صح، جواب إلا كان غلط.' },
  { broken: '=MAX(___)', full: '=MAX(A1:A5)', hint: 'باغي أكبر نقطة ديال سكينة من A1 حتى A5.', options: ['A1:A5', 'A:A', 'A1+A5', '1:5'], correctIndex: 0, explanation: 'MAX(A1:A5) كتعطي أكبر رقم بين A1 و A5.' },
  { broken: '=NB(___)', full: '=NB(A1:A20)', hint: 'NB كتعد الخلايا اللي فيها أرقام.', options: ['A1:A20', 'A1,A20', '"A1:A20"', 'A1-A20'], correctIndex: 0, explanation: 'NB(A1:A20) كتعد شحال من خلية رقمية فهاد المجموعة.' },
  { broken: '=SI(B1<___, "Échoué", "Reçu")', full: '=SI(B1<10,"Échoué","Reçu")', hint: 'فالنقط على 20، أقل من 10 يعني ما نجحش.', options: ['10', '0', '20', '100'], correctIndex: 0, explanation: 'إيلا النقطة أقل من 10 كيعطي "Échoué"، وإلا كيعطي "Reçu".' },
  { broken: '=SOMME(A1:A5)+___', full: '=SOMME(A1:A5)+SOMME(B1:B5)', hint: 'عند نادية نقط فعمود B، جمعهم بنفس الطريقة.', options: ['SOMME(B1:B5)', 'B1:B5', 'MOYENNE(B1:B5)', 'MAX(B1:B5)'], correctIndex: 0, explanation: 'كنجمعو مجموع A ومجموع B باش نجيبو المجموع كامل.' },
];
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Pressable android_ripple={{ color: 'rgba(0,0,0,0.1)' }} onPress={onBack} style={styles.backBtn}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <Animated.View style={{ opacity: fadeAnim, gap: spacing.lg }}>
            ...
            {current.options.map((opt, idx) => {
              ...
              return (
                <Pressable android_ripple={{ color: 'rgba(0,0,0,0.1)' }}
                  ...
                  onPress={() => handleAnswer(idx)} disabled={chosen !== null}
                >
          ...
          {showExplanation && (
            <View style={styles.explanationBlock}>
              <Text style={styles.explanationTitle}>
                {chosen === current.correctIndex ? '✅ صح!' : '❌ غلط!'}
              </Text>
              <Text style={styles.explanationLabel}>الفورميل الصحيحة:</Text>
          ...
          {chosen !== null && (
            <Pressable android_ripple={{ color: 'rgba(0,0,0,0.1)' }} style={styles.nextBtn} onPress={handleNext}>
