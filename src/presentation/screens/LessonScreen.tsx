  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.progressDots}>
        {slides.map((_, i) => (
          <Pressable android_ripple={{ color: 'rgba(0,0,0,0.1)' }} key={i} onPress={() => setSlideIdx(i)}>
      ...
      <View style={styles.footer}>
        <Pressable android_ripple={{ color: 'rgba(0,0,0,0.1)' }}
          ...
          disabled={slideIdx === 0}
        >
          <Text style={[styles.prevText, slideIdx === 0 && styles.btnTextDisabled]}>← ارجع</Text>
        </Pressable>
        <Pressable android_ripple={{ color: 'rgba(0,0,0,0.1)' }} style={styles.nextBtn} onPress={handleNext}>
          <Text style={styles.nextText}>{isLast ? '📝 ابدا التمارين!' : 'فهمت! →'}</Text>
        </Pressable>
      </View>
    </View>
  );
