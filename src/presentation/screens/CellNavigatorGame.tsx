...
export function CellNavigatorGame({ onComplete, onBack }: Props) {
  ...
  const nextRound = useCallback(() => {
    if (round + 1 >= TOTAL_ROUNDS) {
      onComplete(Math.min(100, Math.round((scoreRef.current / (TOTAL_ROUNDS * 100)) * 100)));
      return;
    }
  ...
  const handleAnswer = (chosen: string) => {
    ...
    if (isCorrect) {
      ...
      setStreak(newStreak);
      setScore(s => { const n = s + 50 + timeBonus + (newStreak >= 3 ? 20 : 0); scoreRef.current = n; return n; });
    } else {
      setStreak(0);
    }
          ...
          <View style={[styles.feedbackBox, answered === correctStr ? styles.feedbackCorrect : styles.feedbackWrong]}>
            <Text style={styles.feedbackText}>
              {answered === '__timeout__'
                ? `${S.gameCellTimeoutMsg} ${correctStr}`
                : answered === correctStr
                  ? `${S.gameCellCorrectMsg} ${correctStr}${streak >= 3 ? ` 🔥 ${S.gameStreakLabel}` : ''}`
                  : `${S.gameCellWrongMsg} ${correctStr}`}
            </Text>
          </View>
        )}
      </Animated.View>
