...
export class SaveProgressUseCase {
  ...
  async execute(
    ...
    const existing = await this.progressRepo.getLessonResult(studentId, lessonId);
    const stars = this.calculateStars(score);

    // Award XP only on first completion; on replay, only if stars improve
    let xpEarned = 0;
    if (!existing) {
      xpEarned = this.calculateXP(score, stars);
    } else if (stars > existing.stars) {
      xpEarned = Math.max(0, this.calculateXP(score, stars) - this.calculateXP(existing.score, existing.stars));
    }
    ...
    if (xpEarned === 0) return { xpEarned: 0, newBadges: [], leveledUp: false, newLevel: progress.level };
