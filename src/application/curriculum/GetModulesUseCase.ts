  ...
  async execute(studentId: string): Promise<ModuleWithProgress[]> {
    ...
    for (let i = 0; i < CURRICULUM.length; i++) {
      ...
      if (i > 0) {
        ...
        const prevResults = allResults.filter(r => r.moduleId === prevModule.id);
        const allLessonsDone = prevResults.length >= prevModule.lessons.length;
        const prevProgress = await this.progressRepo.getModuleProgress(studentId, prevModule.id);
        isUnlocked = allLessonsDone && prevProgress >= CURRICULUM[i].unlockRequirement;
      }
