...
function getOptions(ex: Exercise): string[] {
  ...
  if (ex.type === 'true_false') return ['صح ✓', 'غلط ✗'];
  return [];
}
...
export function SpeedQuizGame({ onComplete, onBack }: Props) {
  ...
  const advanceQuestion = useCallback(() => {
    if (qIdx + 1 >= TOTAL_QUESTIONS) {
      onComplete(Math.round((scoreRef.current / (TOTAL_QUESTIONS * 150)) * 100));
      return;
    }
    Animated.sequence([
  ...
  }, [qIdx, onComplete, fadeAnim, timerAnim]);
  ...
  const handleAnswer = (idx: number) => {
    ...
    if (isCorrect) {
      ...
      setScore(s => { const n = s + 100 + timeBonus + streakBonus; scoreRef.current = n; return n; });
    } else {
      setStreak(0);
    }
      ...
      <Animated.View style={[styles.content, { opacity: fadeAnim }]}>
        <View style={styles.questionCard}>
          ...
          {streak >= 3 && (
            <View style={styles.streakBanner}>
              <Text style={styles.streakText}>🔥 x{streak} {S.gameStreakLabel} +30 {S.gameXpBonus}</Text>
            </View>
          )}
        </View>
        ...
        {chosen === -1 && (
          <View style={styles.timeoutBox}>
            <Text style={styles.timeoutText}>{S.gameTimeoutMsg} {options[correctIdx]}</Text>
          </View>
        )}
      </Animated.View>
